...
csv_file="csv/create_filtered_L.csv"
...
for L in "${L_values[@]}"; do
  ...
  ./bin/main -f dummy -k "$K" -L "$L" -R 40 -a 1.2 -v filtered -x create | while read -r K_val L_val R Rstitched a_val UserTime ElapsedTime; do
    # Append each line to the CSV file
    echo "$K_val,$L_val,$R,$Rstitched,$a_val,$UserTime,$ElapsedTime" >> "$csv_file"
  done
done
