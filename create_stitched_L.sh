...
for L in "${L_values[@]}"; do
  ...
  if [$L -le 100 ]
    ./bin/main -f dummy -k "$L" -L "$L" -R 42 -a 1.2 -Rst 96 -v filtered -x create | while read -r K L R Rstitched a_val UserTime ElapsedTime; do
  else
    ./bin/main -f dummy -k 100 -L "$L" -R 42 -a 1.2 -Rst 96 -v filtered -x create | while read -r K L R Rstitched a_val UserTime ElapsedTime; do
  fi
