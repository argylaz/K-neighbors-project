...
void test_create() {
    TestGraph<vector<int>> G1(true); // Create a directed graph
    TestGraph<vector<int>> G2(false); // Create an undirected graph
...
void test_add_vertex() {
    ...
    // Get vertex set
    unordered_set<vector<int>, OptimizedVectorHash<vector<int>>> V1 = G1.get_vertices();

    // Get adjacency lists
    vector<vector<vector<int>>> L1 = G1.get_private_adjacency_list();
    ...
    auto it = V1.begin();
    for (int i = 0; i < NUM; i++) {
        vector<int> expectedVertex = {i};
        TEST_ASSERT(V1.find(expectedVertex) != V1.end());
        TEST_ASSERT(L1[i][0] == vector<int>{i});         // The first element of each row is the vertex corresponding to the index
        it++;
    }
...
void test_insert_sorted_in_adj_list() {
    ...
    vector<vector<int>> adj = G.get_private_adjacency_list()[5];
    ...
    vector<vector<int>> test_adj = {{5}, {1}, {4}, {5}, {7}};
...
void test_add_edge() {
    ...
    TestGraph<vector<int>> G1(true);
    TestGraph<vector<int>> G2(false);
    ...
    for (int i = 1; i < NUM; i++) {
        ...
        vector<vector<vector<int>>> L1 = G1.get_private_adjacency_list();
        vector<vector<vector<int>>> L2 = G2.get_private_adjacency_list();

        // Check that the adjacency list has the correct entries
        TEST_ASSERT(L1[i-1][1] == vector<int>{i});  
        TEST_ASSERT(L2[i][1] == vector<int>{i-1});
        
        if( i != 1 ) {
            TEST_ASSERT(L2[i-1][2] == vector<int>{i});
...
void test_vertex_to_index() {
    ...
    for (int i = 0; i < NUM; i++) {
        TEST_ASSERT(G1.get_vertex_from_index(i) == vector<int>{i}); // In this case vertices and indices are all ints
    }
}
